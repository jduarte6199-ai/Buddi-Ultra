...
struct ContentView: View {
    ...
    private var computedChinWidth: CGFloat {
        ...
        {
            chinWidth = 640
        } else if (!coordinator.expandingView.show || coordinator.expandingView.type == .buddy)
            && vm.notchState == .closed && hasActiveClaudeSession && !vm.hideOnClosed
        {
    ...
    var body: some View {
        ...
        ZStack(alignment: .top) {
            VStack(spacing: 0) {
                ...
                mainLayout
                    .frame(height: vm.notchState == .open
                           ? vm.notchSize.height
                           : coordinator.helloAnimationRunning ? 150 : nil)
                    .conditionalModifier(true) { view in
                        ...
                        return view
                            ...
                            .animation(.spring(response: 0.3, dampingFraction: 0.85), value: vm.notchSize.height)
        ...
        .padding(.bottom, 8)
        .frame(maxWidth: openNotchSize.width, maxHeight: .infinity, alignment: .top)
        .compositingGroup()
        .scaleEffect(
            x: gestureScale,
            y: gestureScale,
                        ...
                        && vm.notchState == .closed && Defaults[.showPowerStatusNotifications]
                    {
                        ...
                        .frame(height: vm.effectiveClosedNotchHeight, alignment: .center)
                          ...
                          InlineHUD(type: $coordinator.sneakPeek.type, value: $coordinator.sneakPeek.value, icon: $coordinator.sneakPeek.icon, hoverAnimation: $isHovering, gestureProgress: $gestureProgress)
                              .transition(.opacity)
                      } else if (!coordinator.expandingView.show || coordinator.expandingView.type == .buddy) && vm.notchState == .closed && hasActiveClaudeSession && !vm.hideOnClosed {
                          BuddyLiveActivity()
    ...
    @ViewBuilder
    func BuddiFaceAnimation() -> some View {
        HStack {
            HStack {
                Rectangle()
                    .fill(.clear)
    ...
    var dragDetector: some View {
        if Defaults[.shelf] && vm.notchState == .closed {
            Color.clear
                .frame(maxWidth: .infinity, maxHeight: vm.closedNotchSize.height + 50)
                .contentShape(Rectangle())
...
extension ContentView {
    private func BuddyLiveActivity() -> some View {
        ...
        return HStack(spacing: 0) {
            HStack(spacing: 2) {
                BuddyFaceIcon(fontSize: min(notchContentHeight * 0.4, 9))
                if hasPending {
                    PermissionIndicatorIcon(
                        size: min(notchContentHeight * 0.5, 12),
                        color: TerminalColors.amber
                    )
                }
            }
            .frame(width: sideWidth + (hasPending ? 18 : 0), height: notchContentHeight)
            ...
            Group {
                if isProcessing || hasPending {
                    ...
                    ReadyForInputIndicatorIcon(
                        size: min(notchContentHeight * 0.6, 14)
                    )
                }
            }
            .frame(width: sideWidth, height: notchContentHeight)
        }
