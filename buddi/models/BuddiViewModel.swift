...
class BuddiViewModel: NSObject, ObservableObject {
    @ObservedObject var coordinator = BuddiViewCoordinator.shared
    @ObservedObject var detector = FullscreenMediaDetector.shared

    let animationLibrary: BuddiAnimations = .init()
    ...
    // True while the cursor is over the now-playing media player. Used so a horizontal
    // two-finger swipe over the player skips tracks instead of switching menu tabs.
    ...
    var isScrollGestureActive: Bool { !scrollGestureSuppressionTokens.isEmpty }
    ...
    private func setupDetectorObserver() {
        ...
        Publishers.CombineLatest(coordinator.$currentView, coordinator.$buddyPanelHeight)
            .sink { [weak self] view, height in
    ...
    func open() {
        ...
        if coordinator.currentView == .buddy {
            self.notchSize = CGSize(width: openNotchSize.width, height: coordinator.buddyPanelHeight)
        } else {
            self.notchSize = openNotchSize
        }
